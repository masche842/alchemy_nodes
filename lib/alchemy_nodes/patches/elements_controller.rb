    ...
    module ElementsController
      ...
      def load_container_to_page
        if params[:container_id].present?
          ...
          container_type = 'AlchemyNodes::Container'
          container_id = params[:element][:container_id]
        elsif (container_id = (params[:page_id].presence or params[:element] and params[:element][:page_id])).present?
          container_type = 'Alchemy::Page'
          @page = Alchemy::Page.find(container_id)
        else
          raise AlchemyNodes::Node::NoContainerError
        end
        container_model = container_type.classify.constantize
        @page ||= container_model.includes(:elements => :contents).find(container_id)
      end
    end
  end
end

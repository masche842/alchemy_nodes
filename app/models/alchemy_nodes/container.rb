...
module AlchemyNodes
  class Container < ActiveRecord::Base
    ...
    has_many :cells, :dependent => :destroy, :class_name => 'Alchemy::Cell'
    has_many :elements, :dependent => :destroy, :order => :position, :class_name => 'Alchemy::Element'
    has_and_belongs_to_many :to_be_sweeped_elements_for_containers, :class_name => 'Alchemy::Element', :uniq => true, :join_table => 'alchemy_elements_containers'

    #validates_presence_of :name, :message => '^' + I18n.t("please enter a name")

    send :attr_accessor, :do_not_autogenerate
    send :attr_accessor, :do_not_sweep
    send :attr_accessor, :do_not_validate_language
